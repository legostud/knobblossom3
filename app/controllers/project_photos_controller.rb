class ProjectPhotosController < ApplicationController
  ...
  def index
    ...
    @project_photos.each do |project_photo|
      project_photo[:project_name] = Project.select(:name).where(:id => project_photo[:project_id].to_i)
      project_photo[:photo_name] = Photo.select(:caption).where(:id => project_photo[:photo_id].to_i)
    end
